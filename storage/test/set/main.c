...
#define MULTIPLETABLE_CONF			"default_mixed.conf"	// Server configuration file with complex table.
...
START_TEST (test_error_twocomma) //Added 1
{
	int serverpid = start_server("conf-twocomma.conf", NULL, "test_error_twocomma.serverout");
	fail_unless(serverpid == -1, "Server should exit due to error in config file.");
}
END_TEST

START_TEST (test_error_multiplecomma) //Added 2
{
	int serverpid = start_server("conf-multiplecomma.conf", NULL, "test_error_multiplecomma.serverout");
	fail_unless(serverpid == -1, "Server should exit due to error in config file.");
}
END_TEST
...
START_TEST (test_error_doublenegsize) //Added 5
{
	int serverpid = start_server("conf-doublenegsize.conf", NULL, "test_error_doublenegsize.serverout");
	fail_unless(serverpid == -1, "Server should exit due to error in config file.");
}
END_TEST
...
START_TEST (test_wronghost) //Added 10
{
	void *conn = storage_connect("wronghost", server_port);
	fail_unless(conn == NULL, "storage_connect with invalid param should fail.");
	fail_unless(errno != 0, "storage_connect with invalid param not setting errno properly.");
}
END_TEST


START_TEST (test_wrongport) //Added 11
{
	void *conn = storage_connect(SERVERHOST, (server_port+1));
	fail_unless(conn == NULL, "storage_connect with invalid param should fail.");
	fail_unless(errno != 8, "storage_connect with invalid param not setting errno properly.");
}
END_TEST
...
START_TEST (test_setget_specialint)
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
...
START_TEST (test_setget_largeint) // Added 7 
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a set
	strncpy(record.value, "col 500", sizeof record.value);
	int status = storage_set(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
	...
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 500, "Got wrong value.");
}
END_TEST


START_TEST (test_setget_superlargeint) // Added 8 
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 5000, "Got wrong value.");
}
END_TEST

START_TEST (test_setget_settwotimesandget) // Added 9 
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 5001, "Got wrong value.");
}
END_TEST
...
START_TEST (test_setget_setMultipleTimesAndGetMultipleTimes) // Added 11
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
...
START_TEST (GetSetGetSetExtremelyLargeTimes) // Added hardest
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do several sets
	strncpy(record.value, "col 5005", sizeof record.value);
	storage_set(INTTABLE, KEY, &record, test_conn);
	strncpy(record.value, "col 5006", sizeof record.value);
	storage_set(INTTABLE, KEY, &record, test_conn);
	strncpy(record.value, "col 5007", sizeof record.value);
	storage_set(INTTABLE, KEY, &record, test_conn);
	strncpy(record.value, "col 5008", sizeof record.value);
	storage_set(INTTABLE, KEY, &record, test_conn);
	...
	strncpy(record.value, "col 5009", sizeof record.value);
	int status = storage_set(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
	...
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 5009, "Got wrong value.");
}
END_TEST
...
START_TEST (test_setget_posfloat) // Added 17
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a set
	strncpy(record.value, "col 2.1", sizeof record.value);
	int status = storage_set(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
	...
	fail_unless(fields == 1 && intval == 2.1, "Got wrong value.");
}
END_TEST
...
START_TEST (test_setget_superlargefloat) // Added 19 
{
	struct storage_record record;
	int fields = 0;
	int intval = 0;
	...
	// Do a set
	strncpy(record.value, "col 5000.12", sizeof record.value);
	int status = storage_set(INTTABLE, KEY, &record, test_conn);
	fail_unless(status == 0, "Error setting a key/value pair.");
	...
	fail_unless(status == 0, "Error getting a value.");
	fields = sscanf(record.value, "col %d", &intval);
	fail_unless(fields == 1 && intval == 5000.12, "Got wrong value.");
}
END_TEST
...
START_TEST (test_query_notint) //Added 14
{
	// Do a query.  Expect one match.
	int foundkeys = storage_query(INTTABLE, "col>notanumber", test_keys, MAX_RECORDS_PER_TABLE, test_conn);
...
START_TEST (test_query_withspace) //Added 15
{
...
*/
	// Config file tests
	tc = tcase_create("TEAM TEST:\n testing different circumstances");
	tcase_set_timeout(tc, TESTTIMEOUT);
	/////tcase_add_test(tc, test_error_twocomma);  //1
	/////tcase_add_test(tc, test_error_multiplecomma);  //2
	/////tcase_add_test(tc, test_error_notcomma); //3
	/////tcase_add_test(tc, test_error_mixed); //4
	/////tcase_add_test(tc, test_error_doublenegsize); //5
	/////tcase_add_test(tc, test_error_alotnegsize); //6
	/////suite_add_tcase(s, tc);

	// Config file tests
	tc = tcase_create("TEAM TEST:\n more complicated circumstances");
	tcase_set_timeout(tc, TESTTIMEOUT);
	///ftcase_add_test(tc, test_config_twotable); //9
	suite_add_tcase(s, tc);
	...
	tc = tcase_create("TEAM TEST:\n wrong configurations test");
	tcase_set_timeout(tc, TESTTIMEOUT);
	/////tcase_add_test(tc, test_wronghost); //10
	///tcase_add_test(tc, test_wrongport); //11
	suite_add_tcase(s, tc);

	// Get tests with invalid parameters
	tc = tcase_create("TEAM TEST:\n really hard example test");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	/////tcase_add_test(tc, test_toughbadtable); //12
	suite_add_tcase(s, tc);
	...
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	///tcase_add_test(tc, test_getnumberbasedtable); //14
	suite_add_tcase(s, tc);
/*
...
*/
	...
	tc = tcase_create("TEAM TEST:\n get set with multiple circumstances");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	/////tcase_add_test(tc, test_setget_specialint);
	/////tcase_add_test(tc, test_setget_largeint); //7
	/////tcase_add_test(tc, test_setget_superlargeint); //8
	tcase_add_test(tc, test_setget_settwotimesandget); //9
	tcase_add_test(tc, test_setget_setmultipletimesandget); //10
	tcase_add_test(tc, test_setget_setMultipleTimesAndGetMultipleTimes); //11
	tcase_add_test(tc, GetSetGetSetManyTimes); //12
	tcase_add_test(tc, GetSetGetSetExtremelyLargeTimes); //hardest. Modifying an existed record several times
	tcase_add_test(tc, DeletionTest); //For deleting

	/////tcase_add_test(tc, test_setget_posfloat); //17
	/////tcase_add_test(tc, test_setget_largefloat); //18
	/////tcase_add_test(tc, test_setget_superlargefloat); //19

	suite_add_tcase(s, tc);

	// Query tests on simple tables
	tc = tcase_create("TEAM TEST:\n several really hard query predicates");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple_populate, test_teardown);
	/////tcase_add_test(tc, test_query_bigint); //13
	/////tcase_add_test(tc, test_query_notint); //14
	/////tcase_add_test(tc, test_query_withspace); //15
	/////tcase_add_test(tc, test_query_withmultiplespace); //16
