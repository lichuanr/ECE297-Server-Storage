...
/****************Get test: invalid table/key/record/conn parameter**************/
START_TEST (test_getinvlaid_invalidtable)
{
	struct storage_record record;
...
START_TEST (test_getinvlaid_invalidkey)
{
	struct storage_record record;
...
START_TEST (test_getinvlaid_invalidrecord)
{
	struct storage_record record;
...
START_TEST (test_getinvlaid_invalidconn)
{
	struct storage_record record;
	int status = storage_get(INTTABLE, KEY1, &record, NULL);
	fail_unless(status == -1, "storage_get with invalid param should fail.");
	fail_unless(errno == ERR_INVALID_PARAM, "storage_get with invalid param not setting errno properly.");
}
END_TEST
...
{
	struct storage_record record;
	int status = storage_get(BADTABLE, KEY1, &record, test_conn);
...
{
	struct storage_record record;
	int status = storage_get(MISSINGTABLE, KEY1, &record, test_conn);
...
/**************************Get test: Get on simple table**************************/
START_TEST (test_get_posint)
{
	struct storage_record record;
		int fields = 0;
		int intval = 0;
...
int main(int argc, char *argv[])
{
	...
	// Get tests with invalid parameter
	tc = tcase_create("getinvalid");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	tcase_add_test(tc, test_getinvlaid_invalidtable);
	tcase_add_test(tc, test_getinvlaid_invalidkey);
	tcase_add_test(tc, test_getinvlaid_invalidrecord);
	tcase_add_test(tc, test_getinvlaid_invalidconn);
	suite_add_tcase(s, tc);

	// Get tests with bad parameters
	tc = tcase_create("getbad");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	...
	// Get tests with missing parameters
	...
	// Get test on simple tables
	tc = tcase_create("get");
	tcase_set_timeout(tc, TESTTIMEOUT);
	tcase_add_checked_fixture(tc, test_setup_simple, test_teardown);
	tcase_add_test(tc, test_get_posint);
	suite_add_tcase(s, tc);
